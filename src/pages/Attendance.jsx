...
export default function Attendance() {
    ...
    const fetchData = async () => {
        try {
            const response = await AttendanceUsers.getAttendance(currentPage, totalItems, search);
    ...
    const handleDeleteServiceTypeConfirm = async (id) => {
        try {
            await AttendanceUsers.deleteLocation(id);
            handleCloseModal();
            fetchData();
        } catch (error) {
                    ...
                    <table className='table table-zebra table-xs table-pin-rows table-pin-cols text-xs cursor-pointer'>
                        <thead className=''>
                            <tr>
                                ...
                                <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[20%]'>User</th>
                                <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[10%]'>Role</th>
                                <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[10%]'>Check In</th>
                                <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[10%]'>Check Out</th>
                                <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[10%]'>Status</th>
                        ...
                        <tbody>
                            {data.map((service, index) => (
                                <tr key={index}>
                                    ...
                                    <td className='py-3 px-3'>{service.User?.Role}</td>
                                    <td className='py-3 px-3'>{format(service.InDate, "dd/MM/yyyy HH:mm:ss")}</td>
                                    <td className='py-3 px-3'>{service.OutDate !== null ? format(service.OutDate, "dd/MM/yyyy HH:mm:ss") : "-"}</td>
                                    <td className='py-3 px-3'>{service.Status}</td>
                                    <td className='py-3 px-3'>
                                        <div className="flex flex-row space-x-3 justify-center items-center">
