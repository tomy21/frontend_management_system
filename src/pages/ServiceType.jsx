...
import { serviceType } from '../Utils/apiManageClient';
import { format } from 'date-fns';
import { FaArrowLeftLong, FaArrowRightLong } from 'react-icons/fa6';
import ServiceTypeModal from '../component/modal/ServiceTypeModal';
...
export default function ServiceType() {
  ...
  const fetchData = async () => {
    try {
      const response = await serviceType.getAll(currentPage, totalItems, search);
  ...
  const handleSaveServiceType = async (serviceData) => {
    try {
      if (selectedService) {
        // Update existing service
        await serviceType.update(selectedService.Id, serviceData);
      } else {
        // Add new service
        ...
        <table className='table table-zebra table-xs table-pin-rows table-pin-cols text-xs cursor-pointer'>
          <thead className=''>
            <tr>
              ...
              <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[20%]'>Service Type</th>
              <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[10%]'>Initial</th>
              <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[20%]'>Description</th>
              <th className='p-3 border-b-2 text-black border-gray-700 bg-gray-300 w-[15%]'>Create Date</th>
          ...
          <tbody>
            {data.map((service, index) => (
              <tr key={index}>
                ...
                <td className='py-3 px-3'>{service.TypeName}</td>
                <td className='py-3 px-3'>{service.Initial}</td>
                <td className='py-3 px-3'>{service.Description}</td>
                <td className='py-3 px-3'>{format(new Date(service.CreatedAt), 'dd MMMM yyyy')}</td>
